...
def get_model():
    ...
    model_client = AzureAI(app_config).get_client_autogen()
    # Ensure configuration options are loaded correctly
    if not model_client:
        raise ValueError('Model client initialization failed. Check AzureAI and AppConfig settings.')
    return model_client
